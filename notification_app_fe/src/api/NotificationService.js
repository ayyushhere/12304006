...
export const fetchNotifications = async ({ limit, page, notification_type } = {}) => {
  ...
  let result = [...hardcodedNotifications];
  
  if (notification_type && notification_type !== 'All') {
    result = result.filter(n => n.Type === notification_type);
  }
  
  if (limit) {
    // Note: The limit here applies directly to the returned list for simplicity
